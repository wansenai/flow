...
case "$2" in
  ...
  "test")
    ENVIRONMENT=test
    PROPERTIES=classpath:/application.properties,/etc/config/flowmanager/application-test.properties
    ;;
  "prod")
    ENVIRONMENT=prod
    PROPERTIES=classpath:/application.properties,/etc/config/flowmanager/application-prod.properties
    ;;
  *)
    if [[ -n $2 ]]; then
